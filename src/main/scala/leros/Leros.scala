...
/** Instruction memory. Contains the register for using the on-chip ROM. Uses
  * Chisel synchronous reset to also execute the first instruction.
  *
...
/** Leros top level.
  *
  * For now do a first sequential implementation. Later have Leros as abstract
  * class with base state and several implementations (different pipelines).
  */
class Leros(size: Int, memSize: Int, prog: String, fmaxReg: Boolean)
    extends Module {
  val io = IO(new Bundle {
  ...
  val dataRead =
    dataMem.read(Mux(decReg.isLoadAddr && stateReg === exe, accu, addrReg))
  ...
  alu.io.din := Mux(
    decReg.isLoadInd,
    dataRead,
    Mux(decReg.isRegOpd, registerRead, opdReg)
  )
